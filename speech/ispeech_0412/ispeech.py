...
from ispeech import create_app, socketio
...
def start_recording(message):
    ...
    wf.setnchannels(message.get('numChannels', 1))
    wf.setsampwidth(message.get('bps', 16) // 8)
    wf.setframerate(message.get('fps', 44100))
    session['wavefile'] = wf
...
def write_audio(message):
    ...
    session['wavefile'].writeframes(message)


@socketio.on('end-recording', namespace='/audio')
def end_recording(message):
    """Stop recording audio from the client."""
    emit('add-wavefile', url_for('static',
                                 filename='data/' + session['wavename']))
